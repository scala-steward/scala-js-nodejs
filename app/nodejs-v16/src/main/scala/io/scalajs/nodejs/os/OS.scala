...
trait OS extends js.Object {
  ...
  /** Returns an object containing commonly used operating system specific constants for error codes, process signals,
    * and so on. The specific constants currently defined are described in OS Constants.
    * @see
    *   https://nodejs.org/api/os.html#os_os_constants_1
    */
  def constants: OSConstants = js.native

  /** A constant defining the appropriate End-of-line marker for the operating system. <ul> <li>\n on POSIX</li>
    * <li>\r\n on Windows</li> </ul>
    * @example
    *   os.EOL
    */
  ...
  /** Returns the operating system CPU architecture. Possible values are 'x64', 'arm' and 'ia32'. Returns the value of
    * process.arch.
    * @example
    *   os.arch()
    */
  ...
  /** Returns an array of objects containing information about each CPU/core installed: model, speed (in MHz), and times
    * (an object containing the number of milliseconds the CPU/core spent in: user, nice, sys, idle, and irq).
    * @example
    *   os.cpus()
    */
  ...
  /** Returns an array containing the 1, 5, and 15 minute load averages.
    ...
    * number. As a rule of thumb, the load average should ideally be less than the number of logical CPUs in the system.
    *
    * The load average is a very UNIX-y concept; there is no real equivalent on Windows platforms. That is why this
    * function always returns [0, 0, 0] on Windows.
    * @example
    *   os.loadavg()
    */
  ...
  /** The os.userInfo() method returns information about the currently effective user -- on POSIX platforms, this is
    ...
    *
    * The value of homedir returned by os.userInfo() is provided by the operating system. This differs from the result
    * of os.homedir(), which queries several environment variables for the home directory before falling back to the
    * operating system response.
    * @return
    *   a [[UserInfoObject user information object]]
    */
  def userInfo(options: UserInfoOptions): UserInfoObject = js.native
  def userInfo(): UserInfoObject                         = js.native

  /** Returns a string identifying the kernel version. Added in: v13.11.0
    */
