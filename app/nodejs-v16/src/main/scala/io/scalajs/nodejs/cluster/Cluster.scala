...
trait Cluster extends IEventEmitter {
  ...
  /** The scheduling policy, either cluster.SCHED_RR for round-robin or cluster.SCHED_NONE to leave it to the operating
    * system. This is a global setting and effectively frozen once you spawn the first worker or call
  ...
  /** After calling .setupMaster() (or .fork()) this settings object will contain the settings, including the default
    * values. It is effectively frozen after being set, because .setupMaster() can only be called once. This object is
    * not supposed to be changed or set manually, by you.
    */
  ...
  /** A hash that stores the active worker objects, keyed by id field. Makes it easy to loop through all the workers. It
    * is only available in the master process.
    *
    * A worker is removed from cluster.workers after the worker has disconnected and exited. The order between these two
    * events cannot be determined in advance. However, it is guaranteed that the removal from the cluster.workers list
    * happens before last 'disconnect' or 'exit' event is emitted.
    */
  ...
  /** setupMaster is used to change the default 'fork' behavior. Once called, the settings will be present in
    * cluster.settings. Note that: <ul> <li>any settings changes only affect future calls to .fork() and have no effect
    * on workers that are already running</li> <li>The only attribute of a worker that cannot be set via .setupMaster()
    * is the env passed to .fork()</li> <li>the defaults above apply to the first call only, the defaults for later
    * calls is the current value at the time of cluster.setupMaster() is called</li> </ul>
    * @example
