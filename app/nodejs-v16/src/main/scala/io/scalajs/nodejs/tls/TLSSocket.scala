...
/** The tls.TLSSocket is a subclass of net.Socket that performs transparent encryption of written data and all required
  * TLS negotiation.
  *
...
class TLSSocket protected () extends net.Socket {
  ...
  /** Returns an object representing the peer's certificate. The returned object has some properties corresponding to
    ...
    *
    * If the full certificate chain was requested, each certificate will include a issuerCertificate property containing
    * an object representing its issuer's certificate.
    * @param detailed
  ...
  /** Returns a string containing the negotiated SSL/TLS protocol version of the current connection. The value 'unknown'
    * will be returned for connected sockets that have not completed the handshaking process. The value null will be
  ...
  /** Returns the ASN.1 encoded TLS session or undefined if no session was negotiated. Can be used to speed up handshake
    * establishment when reconnecting to the server.
    * @return
  ...
  def getPeerX509Certificate(): X509Certificate = js.native
...
trait TLSSocketOptions extends js.Object {
  ...
  /** The SSL/TLS protocol is asymetrical, TLSSockets must know if they are to behave as a server or a client. If true
  ...
  /** Whether to authenticate the remote peer by requesting a certificate. Clients always request a server certificate.
    * Servers (isServer is true) may optionally set requestCert o true to request a client certificate.
    */
  ...
  /** If true, specifies that the OCSP status request extension will be added to the client hello and an 'OCSPResponse'
  ...
  /** Optional TLS context object created with tls.createSecureContext(). If a secureContext is not provided, one will
    * be created by calling tls.createSecureContext() with no options.
    */
