...
/** The console module provides a simple debugging console that is similar to the JavaScript console mechanism provided
  * by web browsers.
  *
  * The module exports two specific components: <ul> <li>A `Console` class with methods such as `console.log()`,`
  * console.error()` and `console.warn()` that can be used to write to any Node.js stream.</li> <li>A global `console`
  * instance configured to write to `process.stdout` and `process.stderr`. The global `console` can be used without
  * calling `require('console')`.</li> </ul>
  *
  * Warning: The global console object's methods are neither consistently synchronous like the browser APIs they
  * resemble, nor are they consistently asynchronous like all other Node.js streams. See the note on process I/O for
  * more information.
  *
  * @see
...
class Console protected () extends js.Object {
  ...
  /** A simple assertion test that verifies whether `value` is truthy. If it is not, an `AssertionError` is thrown. If
    * provided, the error `message` is formatted using `util.format()` and used as the error message.
    * @param value
  ...
  /** When `stdout` is a TTY, calling `console.clear()` will attempt to clear the TTY. When `stdout` is not a TTY, this
    * method does nothing.
    *
    * The specific operation of `console.clear()` can vary across operating systems and terminal types. For most Linux
    * operating systems, `console.clear()` operates similarly to the clear shell command. On Windows, `console.clear()`
    * will clear only the output in the current terminal viewport for the Node.js binary.
    */
  ...
  /** Uses [[io.scalajs.nodejs.util.Util.inspect()]] on `obj` and prints the resulting string to `stdout`. This function
  ...
  /** This method calls[[log()]] passing it the arguments received. Please note that this method does not produce any
    * XML formatting
    */
  ...
  /** Prints to `stderr` with newline. Multiple arguments can be passed, with the first used as the primary message and
    * all additional used as substitution values similar to `printf(3)` (the arguments are all passed to
    * [[io.scalajs.nodejs.util.Util.format()]].
    *
    * If formatting elements (e.g. `%d`) are not found in the first string then[[io.scalajs.nodejs.util.Util.inspect()]]
    * is called on each argument and the resulting string values are concatenated. See
    * [[io.scalajs.nodejs.util.Util.format()]] for more information.
    */
  ...
  /** Starts a timer that can be used to compute the duration of an operation. Timers are identified by a unique
    * `label`. Use the same `label` when calling [[timeEnd()]] to stop the timer and output the elapsed time in
  ...
  /** This method does not display anything unless used in the inspector. The `console.profile()` method starts a
    * JavaScript CPU profile with an optional label until [[profileEnd()]] is called. The profile is then added to the
    * **Profile** panel of the inspector.
    */
  def profile(label: String): Unit = js.native
  def profile(): Unit              = js.native

  /** This method does not display anything unless used in the inspector. Stops the current JavaScript CPU profiling
    * session if one has been started and prints the report to the **Profiles** panel of the inspector. See
  ...
  /** This method does not display anything unless used in the inspector. The `console.timeStamp()` method adds an event
    * with the label `'label'` to the **Timeline** panel of the inspector.
    */
  def timeStamp(label: String): Unit = js.native
  def timeStamp(): Unit              = js.native
}

/** A global `Console` instance configured to write to [[io.scalajs.nodejs.process.Process.stdout]] and
  * [[io.scalajs.nodejs.process.Process.stderr]].
  */
@js.native
@JSGlobal("console")
