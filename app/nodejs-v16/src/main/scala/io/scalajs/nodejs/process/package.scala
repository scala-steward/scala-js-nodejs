...
package object process {
  ...
  implicit final class ProcessExtensions(private val process: Process) extends AnyVal {
    ...
    /** If process is spawned with an IPC channel, 'disconnect' will be emitted when IPC channel is closed. Read more in
      * child_process 'disconnect' event doc.
      * @param listener
      *   the event listener function
      * @since 0.7.7
      */
    ...
    /** Emitted when the process is about to exit. There is no way to prevent the exiting of the event loop at this
      * point, and once all 'exit' listeners have finished running the process will exit. Therefore you must only
      * perform synchronous operations in this handler. This is a good hook to perform checks on the module's state
      * (like for unit tests). The callback takes one argument, the code the process is exiting with.
      * @param listener
      *   the event listener function
      * @example
    ...
    /** Messages sent by ChildProcess.send() are obtained using the 'message' event on the child's process object.
      * @param listener
      *   the event listener function <ul> <li>message: <Object> a parsed JSON object or primitive value</li>
      *   <li>sendHandle: <Handle object> a net.Socket or net.Server object, or undefined.</li> </ul>
      * @since 0.5.10
      */
    ...
    /** Emitted whenever a Promise was rejected and an error handler was attached to it (for example with
      * promise.catch()) later than after an event loop turn.
      * @param listener
      *   the event listener function
      */
    ...
    /** The 'uncaughtException' event is emitted when an exception bubbles all the way back to the event loop. By
      * default, Node.js handles such exceptions by printing the stack trace to stderr and exiting. Adding a handler for
      * the 'uncaughtException' event overrides this default behavior.
      * @param listener
      *   the event listener function
      */
    ...
    /** Emitted whenever a Promise is rejected and no error handler is attached to the promise within a turn of the
      * event loop. When programming with promises exceptions are encapsulated as rejected promises. Such promises can
      * be caught and handled using promise.catch() and rejections are propagated through a promise chain. This event is
      * useful for detecting and keeping track of promises that were rejected whose rejections were not handled yet.
      * @param listener
      *   the event listener function
      */
