...
/** Instances of the Cipher class are used to encrypt data. The class can be used in one of two ways: <ul> <li>As a
  * stream that is both readable and writable, where plain unencrypted data is written to produce encrypted data on the
  * readable side, or</li> <li>Using the cipher.update() and cipher.final() methods to produce the encrypted data.</li>
  * </ul> The crypto.createCipher() or crypto.createCipheriv() methods are used to create Cipher instances. Cipher
  * objects are not to be created directly using the new keyword.
  */
@js.native
sealed trait Cipher extends Transform {

  /** Returns any remaining enciphered contents. If output_encoding parameter is one of 'binary', 'base64' or 'hex', a
    * string is returned. If an output_encoding is not provided, a Buffer is returned.
    *
    * Once the cipher.final() method has been called, the Cipher object can no longer be used to encrypt data. Attempts
    * to call cipher.final() more than once will result in an error being thrown.
    * @example
  ...
  /** When using an authenticated encryption mode (only GCM is currently supported), the cipher.getAuthTag() method
    ...
    *
    * The cipher.getAuthTag() method should only be called after encryption has been completed using the cipher.final()
    * method.
    * @example
    *   cipher.getAuthTag()
    */
  ...
  /** When using block encryption algorithms, the Cipher class will automatically add padding to the input data to the
    ...
    *
    * When auto_padding is false, the length of the entire input data must be a multiple of the cipher's block size or
    * cipher.final() will throw an Error. Disabling automatic padding is useful for non-standard padding, for instance
  ...
  /** Updates the cipher with data. If the input_encoding argument is given, it's value must be one of 'utf8', 'ascii',
    ...
    *
    * The output_encoding specifies the output format of the enciphered data, and can be 'binary', 'base64' or 'hex'. If
