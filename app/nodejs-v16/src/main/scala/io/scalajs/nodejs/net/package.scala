...
package object net {
  ...
  implicit final class SocketExtensions[T <: Socket](private val socket: T) extends AnyVal {
    ...
    /** Emitted once the socket is fully closed. The argument had_error is a boolean which says if the socket was closed
    ...
    /** Emitted when the other end of the socket sends a FIN packet.
      ...
      * pending write queue. However, by setting allowHalfOpen == true the socket will not automatically end() its side
      * allowing the user to write arbitrary amounts of data, with the caveat that the user is required to end() their
      * side now.
      * @param callback
      *   the callback
      * @example
    ...
    /** Emitted after resolving the hostname but before connecting. Not applicable to UNIX sockets.
      * @param callback
      *   the callback, with the following arguments: <ul> <li>err <Error> | <Null> The error object. See
      *   dns.lookup().</li> <li>address <String> The IP address.</li> <li>family <String> | <Null> The address type.
      *   See dns.lookup().</li> <li>host <String> The hostname.</li> </ul>
      * @example
