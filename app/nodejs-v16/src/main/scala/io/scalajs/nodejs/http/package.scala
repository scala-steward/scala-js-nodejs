...
package object http {
  ...
  implicit final class ServerExtensions[T <: Server](private val server: T) extends AnyVal {
    ...
    /** Emitted each time a request with an http Expect header is received, where the value is not 100-continue. If this
      * event isn't listened for, the server will automatically respond with a 417 Expectation Failed as appropriate.
    ...
    /** When a new TCP stream is established. socket is an object of type net.Socket. Usually users will not want to
      * access this event. In particular, the socket will not emit 'readable' events because of how the protocol parser
      * attaches to the socket. The socket can also be accessed at request.connection.
      *   - socket <stream.Duplex>
      */
    @inline
    ...
    /** Emitted each time a client requests an HTTP upgrade. If this event is not listened for, then clients requesting
      * an upgrade will have their connections closed.
      *
      * After this event is emitted, the request's socket will not have a 'data' event listener, meaning you will need
      * to bind to it in order to handle data sent to the server on that socket.
      *   - request <http.IncomingMessage> Arguments for the HTTP request, as it is in the 'request' event
  ...
  implicit final class ClientRequestExtensions[T <: ClientRequest](private val client: T) extends AnyVal {
    ...
    /** Emitted when a response is received to this request. This event is emitted only once. The response argument will
      * be an instance of http.IncomingMessage.
      *   - response <http.IncomingMessage>
      */
    @inline
